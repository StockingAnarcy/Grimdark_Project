...
{
    public class AttackA1PlayerState : PlayerState
    {
        ...
        protected override void OnEnter(Player player)
        {
            ...
            player.playerEvents.OnAttack?.Invoke(1); // Уведомляем о атаке
        ...
        protected override void OnStep(Player player)
        {
            ...
            if (player.inputs.GetAttackADown() && timeSinceEntered >= player.stats.current.attackA1 * 0.5f &&
                timeSinceEntered <= player.stats.current.attackA1 * 0.9f)
            {
            ...
            // Завершение атаки
            if (timeSinceEntered >= player.stats.current.attackA1)
            {
